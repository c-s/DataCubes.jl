...
facts("Ungroup tests") do
  ...
  @fact ungroup(jaggedarr0, jaggedarr0[1,:]) --> nalift(transpose(hcat(collect(1:9),["a","b","c","hello","hi","this","one","ten","A"])))
  @fact ungroup(jaggedarr0, 2) --> ungroup(jaggedarr0, jaggedarr0[1,:])
  @fact ungroup(nonjaggedarr, jaggedarr0[1,:]) --> nalift(transpose(hcat([1,1,1,3,3,3,3,3,5],[2,2,2,4,4,4,4,4,6])))
  @fact ungroup(transpose(jaggedarr0), transpose(jaggedarr0)[:,1]) --> transpose(ungroup(jaggedarr0, jaggedarr0[1,:]))
  @fact ungroup(transpose(nonjaggedarr), transpose(jaggedarr0)[:,1]) --> transpose(ungroup(nonjaggedarr, jaggedarr0[1,:]))
  ...
  @fact ungroup(jaggedarr, jaggedarr[1,:]) --> nalift(transpose(hcat(collect(1:9),collect(11:19))))
  @fact ungroup(jaggedarr, 2) --> ungroup(jaggedarr, jaggedarr[1,:])
  @fact ungroup(nonjaggedarr, jaggedarr[1,:]) --> nalift(transpose(hcat([1,1,1,3,3,3,3,3,5],[2,2,2,4,4,4,4,4,6])))
  @fact ungroup(transpose(jaggedarr), transpose(jaggedarr)[:,1]) --> transpose(ungroup(jaggedarr, jaggedarr[1,:]))
  @fact ungroup(transpose(nonjaggedarr), transpose(jaggedarr)[:,1]) --> transpose(ungroup(nonjaggedarr, jaggedarr[1,:]))
