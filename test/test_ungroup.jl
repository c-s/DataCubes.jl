...
facts("Ungroup tests") do
  ...
  @fact ungroup(jaggedarr0, jaggedarr0[1:1,:]) --> nalift(permutedims(hcat(collect(1:9),["a","b","c","hello","hi","this","one","ten","A"]), (2,1)))
  @fact ungroup(jaggedarr0, 2) --> ungroup(jaggedarr0, jaggedarr0[1:1,:])
  @fact ungroup(nonjaggedarr, jaggedarr0[1:1,:]) --> nalift(permutedims(hcat([1,1,1,3,3,3,3,3,5],[2,2,2,4,4,4,4,4,6]), [2,1]))
  @fact ungroup(permutedims(jaggedarr0, [2,1]), permutedims(jaggedarr0, [2,1])[:,1]) --> permutedims(ungroup(jaggedarr0, jaggedarr0[1:1,:]), [2,1])
  @fact ungroup(permutedims(nonjaggedarr, [2,1]), permutedims(jaggedarr0, [2,1])[:,1]) --> permutedims(ungroup(nonjaggedarr, jaggedarr0[1:1,:]), [2,1])
  @fact ungroup(map(Nullable, reshape(Any[[1,2,3],["a","b","c"],[4,5,6,7,8],["hello","hi","this","one","ten"],[9],["A"]], (2,3))), 2) --> nalift(permutedims(hcat(collect(1:9),["a","b","c","hello","hi","this","one","ten","A"]), (2,1)))

  jaggedarr = nalift(reshape(nalift(Any[[1,2,3],[11,12,13],[4,5,6,7,8],[14,15,16,17,18],[9],[19]]), (2,3)))
  @fact ungroup(jaggedarr, jaggedarr[1:1,:]) --> nalift(permutedims(hcat(collect(1:9),collect(11:19)), [2,1]))
  @fact ungroup(jaggedarr, 2) --> ungroup(jaggedarr, jaggedarr[1:1,:])
  @fact ungroup(nonjaggedarr, jaggedarr[1:1,:]) --> nalift(permutedims(hcat([1,1,1,3,3,3,3,3,5],[2,2,2,4,4,4,4,4,6]), [2,1]))
  @fact ungroup(permutedims(jaggedarr, [2,1]), permutedims(jaggedarr, [2,1])[:,1]) --> permutedims(ungroup(jaggedarr, jaggedarr[1:1,:]), [2,1])
  @fact ungroup(permutedims(nonjaggedarr, [2,1]), permutedims(jaggedarr, [2,1])[:,1]) --> permutedims(ungroup(nonjaggedarr, jaggedarr[1:1,:]), [2,1])
